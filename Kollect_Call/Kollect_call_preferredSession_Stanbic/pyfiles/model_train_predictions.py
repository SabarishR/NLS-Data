...
def to_kollectCall_preferredSession_xgbClassifier_modelTraining():
    ...
    # Filter only picked call list from all outcomes
    outcome = CF.fn_getpickedcalllist()
    # Reading the required table
    query = app.config['MYSQL_SELECT_CUSTOMERINFORMATION']+ outcome +')'
    df = DBC.mysql_readTable(query)
    ...
    df[app.config['VAR_FREQUENCY_COLUMNNAME']] = df.groupby(['ID',app.config['VAR_SESSION_COLUMNNAME'],'PROMISE_SUCCESS'])[app.config['VAR_SESSION_COLUMNNAME']].transform('count')
    # dropping NA rows
    df = CF.fn_dropna(df)
    # Replacing Customer_Risk_Status Letter with the respective values
    df = CF.fn_replace_dfcolumnvalues(df,app.config['VAR_CUSTOMER_RISK_STATUS'],app.config['VAR_CUSTOMER_RISK_STATUS_ACTUAL'],app.config['VAR_CUSTOMER_RISK_STATUS_REPLACE'])
    # Removing the duplicated values
    df = CF.fn_dropDuplicates(df,["ID",app.config['VAR_SESSION_COLUMNNAME'],"PROMISE_SUCCESS"])
    # pivoting the table using unique id and promise status
    df_pivot = df.pivot_table(index=['ID', 'PROMISE_SUCCESS'], columns=app.config['VAR_SESSION_COLUMNNAME'], values=app.config['VAR_FREQUENCY_COLUMNNAME']).reset_index()
    # fill na
    df_pivot = CF.fn_fillna(df_pivot,0)
    # merge two dataframes
    df =CF.fn_mergeTwo_dataframe(df,df_pivot,['ID','PROMISE_SUCCESS'])
    
    # Removing the duplicated values
    df = CF.fn_dropDuplicates(df,["ID","PROMISE_SUCCESS"])
    ...
    df = CF.fn_dropColumns(df,["ID","TIME_STAMP",app.config['VAR_FREQUENCY_COLUMNNAME'],app.config['VAR_SESSION_COLUMNNAME']])
...
def to_kollectCall_preferredSession_xgbClassifier_modelPredictions(data):
    ...
    userIPjson = {'ID':[data['ID']],'CUSTOMER_RISK_STATUS':int(data['CUSTOMER_RISK_STATUS']),'INDUSTRY':[data['INDUSTRY']],'CUSTOMER_STATUS':[int(data['CUSTOMER_STATUS'])]}   
    #merge json
